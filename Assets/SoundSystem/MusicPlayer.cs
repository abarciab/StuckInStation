...
public class MusicPlayer : MonoBehaviour
{
    ...
    public void FadeOutAllCurrent(float time)
    {
        mainMusicTimeLeft = Mathf.Infinity;
        StartCoroutine(FadeOut(time));
    }

    public void FadeOutMusic(float time = 1)
    {
        mainMusicTimeLeft = Mathf.Infinity;
        StartCoroutine(FadeOut(time, true));
    }
    ...
    IEnumerator FadeOut(float time, bool OnlyMusic = false)
    {
        ...
        while (timePassed < time) {
            ...
            if (ambientLoop && !OnlyMusic) ambientLoop.PercentVolume(Mathf.Lerp(ambientStart, 0, progress));
