...
/*
void PointLight_float(float3 ToLight, out float3 Direction, out float3 Color, out float DistanceAtten, out float ShadowAtten)
{
#if SHADERGRAPH_PREVIEW
    Direction = float3(0.5, 0.5, 0);
    Color = 1;
    ...
    float3 shadowCoord = WorldPos - _WorldSpaceLightPos0;
    float shadowStrength = texCUBE(_MainLightShadowmapTexture, shadowCoord).r;
    ShadowAtten = shadowStrength;
